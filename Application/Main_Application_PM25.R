...
# Load data
PM25_data = read.csv('Data_PM25.csv', header = T)
...
Y = as.matrix(PM25_data[, "winterpm"])
X = PM25_data[, c("prec","wind","tmin","tmax","rhum", "tcdc")]
X = as.matrix(scale(X, scale = TRUE)) 
X = as.matrix(cbind(1, X))
S = cbind(PM25_data$Longtitude, PM25_data$Latitude)
