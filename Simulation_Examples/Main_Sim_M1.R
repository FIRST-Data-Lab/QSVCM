...
dist = "MN" ## choose "MN" for normal mixture error distribution 
            ## choose "t2" for t2 error distribution 

if (dist == "MN"){
  Finv = QmixNorm(q = tau, mu = c(-5, 0, 5), sd = sqrt(c(4, 0.5, 4)), pi = c(0.1, 0.8, 0.1))
  pop.all=as.matrix(read.csv('eg1_pop_mnorm.csv', header=TRUE))    
} else if(dist == "t2"){
  Finv = qt(tau,2)
  pop.all=as.matrix(read.csv('eg1_pop_t2.csv', header=TRUE))
} else{
